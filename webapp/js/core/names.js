...
export const DEFAULT_PLAYER = 'Anon';
export const DEFAULT_BOT = 'Jun';
...
// storage and the model both stay on Jun/Anon FOREVER. she's a
// fine-tune trained on those two literal strings, feed her
// "Aurora" and she has no idea who that is. so the custom names
// exist only in the two places a human looks: the bubble we
// paint, and the box he types in. everything between those points
// is canonical.
const renamed = () => player !== DEFAULT_PLAYER || bot !== DEFAULT_BOT;

function esc(s) { return s.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'); }

// \b is wrong here, plenty of names end in an apostrophe or a
// non-ascii letter and \b puts a boundary in the middle of those.
// so we match the neighbours ourselves and hand them back
// untouched.
...
// the inverse, on everything the user types before it goes
// anywhere near the model or the db. bot first so naming her
// "Anon" doesn't round trip through the player branch and come
// out as the wrong person.
...
// keep half written names out of the reply we show. tokens split
// across token chunks all the time, and "An" rendered NOW can't
// be un-rendered when the "on" lands 40ms later.
const TOKENS = ['{f_playername}', '{f_botname}'];
export function pendingPartial(buf) {
...
// the markup ships with Jun and Anon written into it in about
// twenty places. rather than templating every one, walk it once
// at boot and run the same apply() the chat bubbles use. anything
// under [data-no-rename] is skipped, that's for copy which is
// ABOUT the defaults ("leave blank for the default (Jun)") and
// gets nonsense if you rename it.
