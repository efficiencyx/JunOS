...
window.ChatAPI = (function () {
  ...
  function chat({ messages, model, reasoning, think, outfit_context, conversation_id, idle, ephemeral, client_time, reply_lang }, { onToken, onThinking, onDone, onError, onDebug, onStats, onToolStatus }) {
    ...
    (async () => {
      try {
        const res = await fetch('api/chat.php', {
          ...
          body: JSON.stringify({ messages, model, reasoning, think, outfit_context, conversation_id, idle, ephemeral, client_time, reply_lang }),
          signal: ctrl.signal,
        });
        if (!res.ok || !res.body) {
