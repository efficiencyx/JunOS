...
export function setOnTranscript(fn) { onTranscript = fn || (() => {}); }
...
export function setOnAudio(fn) { onAudio = fn || null; }
export function setOnState(fn) { onState = fn || (() => {}); }
export function setOnBargeIn(fn) { onBargeIn = fn || (() => {}); }
export function setLogger(fn) { onLog = fn || (() => {}); }
export function isEnabled() { return enabled; }
export function getState() { return state; }
export function setBargeIn(v) { bargeIn = !!v; }
export function setHearAll(v) { hearAll = !!v; }
function getHearAll() { return hearAll; }
export function setSilenceMs(v) { silenceMs = Math.max(300, Math.min(2000, v | 0)); }
...
async function ensureMic() {
  ...
  if (settings.echoCancellation !== true) {
    onLog('warn', 'Mic echo cancellation unavailable - use headphones, or '
      + Names.getBot() + ' may interrupt herself.');
  }

  micCtx = new AudioContext({ sampleRate: SAMPLE_RATE, latencyHint: 'interactive' });
  await micCtx.audioWorklet.addModule(new URL('./mic-worklet.js?v=1', import.meta.url).href);
...
function onRms(rms) {
  ...
  const speakingNow = !!TTS.isSpeaking();
...
async function stt(wav) {
  try {
    const res = await api('stt.php?action=stt', {
      method: 'POST',
