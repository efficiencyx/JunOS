# The karaoke sidecar. htdemucs splits a song into vocals and
# backing, whisper then times every word. same server.py as the
# voice sidecar with different deps, no kokoro or pocket-tts, no
# espeak-ng, but a GPU torch by default. splitting a song is the
# one audio job worth real VRAM, a 4 minute track takes minutes
# on a CPU and seconds on a GPU. it gets its own container so the
# voice sidecar can stay CPU only next to the LLM.
...
# torch first from TORCH_INDEX, or something else pulls a
# different build in behind it, and in its own layer so editing
# requirements doesn't redo a multi-GB install. torchaudio comes
# from the same place so its build matches torch's, demucs uses
# it to read audio. UV_HTTP_TIMEOUT goes up for the slow ROCm
# CDN, or a big wheel trips uv's stall timeout.
