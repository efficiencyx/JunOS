# The voice sidecar. Kokoro-82M and pocket-tts do the talking,
# faster-whisper does the listening. this is a CPU build, the
# models are small enough to keep up in real time there, and a
# GPU copy would sit on ~2GB of VRAM that the LLM wants for its
# own layers. the one audio job that really does want a GPU lives
# in docker/karaoke.Dockerfile.
