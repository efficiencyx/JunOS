...
**A tiny privacy note:** Jun asks you *once*, in the app, whether you'd like to share future chats, usage stats and thumbs so we can train, evaluate and potentially publish privacy-tested Jun LoRA adapters - and no chat is shared unless you explicitly say yes. The payload has a random install id rather than your account email, but Cloudflare and the origin necessarily process your connection IP; the origin keeps only a daily-salted hash for 30 days. We won't call the chat anonymous: it's your words, and free-typed words have a habit of being personal. Say no and absolutely nothing changes about how she works. One click under **Settings → Privacy** to change your mind, another to submit a deletion request, and the whole story lives in the [privacy notice](webapp/privacy.html). Never want to be asked at all? `TELEMETRY=off` in `.env`. 🌸

**Hosting Jun for other people?** Then heads up: the moment someone other than you has an account on your install, their chats are personal data and *you're* the one responsible for them, not us. Jun gives you per-account opt-in, withdrawal and an erasure-request path, but the boring parts are still yours: provide your own notice, verify rights requests end to end, keep the box and backups encrypted, don't hand the database to anyone, and if you point `TELEMETRY_ENDPOINT` at your own collector, edit [`webapp/privacy.html`](webapp/privacy.html) so it names *you* instead of us. [`docs/data-protection.md`](docs/data-protection.md) records the known gaps as well as the controls - crib carefully. 🗂️
